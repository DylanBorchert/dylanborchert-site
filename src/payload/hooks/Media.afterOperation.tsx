...
export const MediaAfterOperation: any = [
    async ({ result, operation, req, args }: any) => {
        if (result.url && (operation === "create" || operation === "update")) {
          ...
          if (result.mimeType.includes("image")) {
            ...
            if (result.mimeType.includes("webp")) {
              console.log("[Media] Skipping vibrant color for webp");
              return;
            }
            try {
              const imagePath = `${host}${result.url}`;
              const palette = await Vibrant.from(imagePath).getPalette();
              const prominentColor = Object.values(palette).reduce(
                (acc, curr) => {
                  if (!curr) return acc;
                  if (!acc) return curr;
                  return curr.population > acc.population ? curr : acc;
                },
                null,
              );
              if (!prominentColor) {
              ...
              const prominentLightness = prominentColor.hsl[2] + 0.05;
              ...
              const textBackground: RGB =
                prominentLightness >= 0.5
                  ...
                  : { r: 0, g: 0, b: 0 };
              const colorPalette = {
                Muted: palette.Muted ? rgb2oklch(palette.Muted.rgb) : null,
              ...
              console.log("[Media] Color Palette Set", colorPalette);
            } catch (error) {
              console.error("[Media] Error getting vibrant color", error);
            }
          } else {
            console.log("[Media] Not an image, skipping vibrant color");
          }
        }
    }
]